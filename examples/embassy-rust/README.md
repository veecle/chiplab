...
`#![no_std]` Rust on the [Embassy](https://embassy.dev) async runtime, using the
`embassy-stm32` / `embassy-nrf` HALs and async UART. Each example prints `Hello world!`.
...
cd examples/embassy-rust/<board>    # target is pinned in .cargo/config.toml
cargo build --release
