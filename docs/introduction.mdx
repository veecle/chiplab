...
Chiplab exposes its capabilities to your agent over MCP. You don't call these directly, your agent discovers and uses them automatically based on what you ask it to do.

Chiplab's tool surface is always evolving. Ask Chiplab directly (call the `ask` tool with no arguments) for the current, complete list of what's available.
...
Chiplab works with any MCP-capable coding agent, with step-by-step setup guides for Cursor, OpenCode, Claude Code, Claude Desktop, VS Code, and Codex. Other MCP-capable clients, GitHub Copilot among them, connect the same way, see [Connect an Agent](/agents/overview) for the full list.

All of them connect to Chiplab over HTTP using the MCP protocol. Authentication is handled through a browser-based OAuth 2.0 flow, no manual token management required.
